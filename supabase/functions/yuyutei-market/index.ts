...
interface YuyuteiMarketCandidate {
  source: 'yuyutei';
  mode: 'sell' | 'buy';
  sourceUrl: string;
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (body.action === 'search') {
      ...
      if (!cardNumber) return json({ error: 'cardNumber is required' }, 400);
      const [sell, buy] = await Promise.all([
        searchYuyutei('sell', cardNumber),
        searchYuyutei('buy', cardNumber)
      ]);
      return json({ candidates: [...sell, ...buy] });
    }

    if (body.action === 'refresh') {
      const sourceUrl = String(body.sourceUrl || '').trim();
      if (!isAllowedYuyuteiCardUrl(sourceUrl)) return json({ error: 'invalid Yuyutei card URL' }, 400);
      return json({ result: await fetchYuyuteiDetail(sourceUrl) });
    }

    return json({ error: 'unknown action' }, 400);
  } catch (error) {
    return json({ error: error instanceof Error ? error.message : 'Yuyutei fetch failed' }, 500);
  }
});
