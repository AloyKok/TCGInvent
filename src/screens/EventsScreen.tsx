...
export function EventsScreen() {
  ...
  const mutation = useMutation({
    mutationFn: () => saveEvent(organization.id, { name, date, location }),
    onSuccess: async () => {
      setName('');
      setLocation('');
    ...
    <div className="grid gap-4">
      ...
      <form className="grid gap-3 rounded-lg border border-line bg-white p-3" onSubmit={(event) => { event.preventDefault(); mutation.mutate(); }}>
        ...
        <div className="grid gap-3 min-[400px]:grid-cols-2">
          <Field label="Date"><TextInput type="date" value={date} onChange={(event) => setDate(event.target.value)} required /></Field>
      ...
      <div className="grid gap-2">
        {(query.data || []).map((event) => (
          <div key={event.id} className="rounded-lg border border-line bg-white p-3">
            ...
            <p className="text-sm text-slate-600">{event.date}{event.location ? ` / ${event.location}` : ''}</p>
